...
void cmd_move_con_to_workspace_number(I3_CMD, const char *which, const char *no_auto_back_and_forth) {
    ...
    if (!ws) {
        ws = workspace_get_on_output(output, which);
    }
...
static bool cmd_resize_tiling_width_height(I3_CMD, Con *current, const char *direction, int px, double ppt) {
    ...
    if (ppt != 0.0) {
        ...
        const double pct = (double)px / (double)con_rect_size_in_orientation(current->parent);
        new_current_percent = current->percent + pct;
        new_next_percent = next->percent - pct;
    }
